...
class SGLangRollout(BaseRollout):
    ...
    def _format_feedback_tool_call(self, tool_name: str, question: str, solution: str) -> str:
        payload = {
            "name": tool_name,
            "arguments": {
                "question": question,
    ...
    def _format_feedback_tool_call_by_group(
        self,
        tool_name: str,
        question: str,
        solutions: list[str],
        ...
        payload = {
            ...
            "arguments": {
                "question": question,
                "mode": "group",        # ★ 第一阶段
                "solutions": solutions, # 一题多解
            },
        }
        serialized = json.dumps(payload, ensure_ascii=False)
        return f"<tool_call>\n{serialized}\n</tool_call>"

    def _format_feedback_merged_by_group(
        self,
        tool_name: str,
        question: str,
        feedbacks: list[str],
        ...
        """
        question: 题目文本
        feedbacks: 第一阶段得到的多个 group-level feedback 文本
        """
        payload = {
            "name": tool_name,
            "arguments": {
    ...
    def _build_feedback_prompts_from_failed_batch(
        self,
        failed_batch: DataProto,
        **kwargs,
    ) -> DataProto:
        """
        基于 _collect_all_negative_rollout_prompts 得到的 failed_batch，
        构造专门给 feedback-tool 用的 prompts：
        - non_tensor_batch["raw_prompt"]
        - non_tensor_batch["tools_kwargs"]
        然后可以直接丢给 _preprocess_prompt_to_async_rollout_requests。
        """

        # === 1. 取出 prompt_text / rollout_text ===
        prompt_text_field = failed_batch.non_tensor_batch.get("prompt_text")
        rollout_text_field = failed_batch.non_tensor_batch.get("rollout_text")

        if prompt_text_field is None or rollout_text_field is None:
            raise ValueError(
                "generate_feedbacks expects `prompt_text` and `rollout_text` in non_tensor_batch; "
                "set them before requesting feedback generation."
            )

        prompt_texts = (
            prompt_text_field.tolist()
            if isinstance(prompt_text_field, np.ndarray)
            else list(prompt_text_field)
        )
        rollout_texts = (
            rollout_text_field.tolist()
            if isinstance(rollout_text_field, np.ndarray)
            else list(rollout_text_field)
        )
        if len(prompt_texts) != len(rollout_texts):
            raise ValueError(
                f"Mismatched prompt/rollout counts when building feedback: "
                f"{len(prompt_texts)} vs {len(rollout_texts)}"
            )

        # === 1.5 从原始 rollout 的 messages 里推断 Message 类型 ===
        # failed_batch.non_tensor_batch["messages"] 里存的是 {"messages": req.messages}
        messages_array = failed_batch.non_tensor_batch["messages"]
        sample_messages = messages_array[0]["messages"]
        MessageCls = type(sample_messages[0])

        # === 2. 确定要调用哪个 feedback tool ===
        tool_name = (
            kwargs.get("tool_name")
            or getattr(self.config.multi_turn, "feedback_tool_name", None)
            or "calc_gsm8k_reward"
        )

        # === 3. 为每条样本构造 raw_prompt & tools_kwargs ===
        raw_prompts: list[list] = []
        tools_kwargs_list: list[dict] = []

        for question, solution in zip(prompt_texts, rollout_texts):
            # 3.1 构造 content（和原来模型输出类似）
            tool_call_str = self._format_feedback_tool_call(
                tool_name=tool_name,
                question=question,
                solution=solution,
            )

            # 3.2 用和 _async_rollout_a_request 一样的逻辑解析 tool_call
            normed_content = tool_call_str
            parsed_tool_calls: list[OpenAIFunctionToolCall] = []

            if self._function_call_parser and self._function_call_parser.has_tool_call(tool_call_str):
                try:
                    normed_content, tool_calls = self._function_call_parser.parse_non_stream(tool_call_str)
                except JSONDecodeError:
                    normed_content = tool_call_str
                    tool_calls = []
                except AttributeError:
                ...
                for tc in tool_calls:
                    ...
                    # 跟原逻辑保持一致：有 decode error 的就丢弃
                    if has_decode_error:
                        continue
                    parsed_tool_calls.append(
                        OpenAIFunctionToolCall(
                            id=str(tc.tool_index),
                            function=function,
                        )
                    )

            # 3.3 构造一条 assistant message
            assistant_msg = MessageCls(
                role="assistant",
                content=normed_content,   # 注意这里用的是 parser 正规化后的 content
            )
            # tool_calls 不一定在 __init__ 里支持，保险起见用属性赋值
            if hasattr(assistant_msg, "tool_calls"):
                assistant_msg.tool_calls = parsed_tool_calls if parsed_tool_calls else None

            raw_prompts.append([assistant_msg])

            # 3.4 tools_kwargs：告诉 rollout 有哪些工具及其 kwargs
            tools_kwargs_list.append(
                {
                    tool_name: {
                        "execute_kwargs": {},       # 比如可以放 timeout 等
                        "calc_reward_kwargs": {},   # 如果 tool 里要算额外 reward
                        "release_kwargs": {},       # 清理资源用
                    }
                }
            )

        # === 4. 把构造好的 raw_prompt / tools_kwargs 塞回 failed_batch ===
        failed_batch.non_tensor_batch = dict(failed_batch.non_tensor_batch)
        failed_batch.non_tensor_batch["raw_prompt"] = np.array(raw_prompts, dtype=object)
        failed_batch.non_tensor_batch["tools_kwargs"] = np.array(tools_kwargs_list, dtype=object)

        # meta_info 打标记
        failed_batch.meta_info = dict(failed_batch.meta_info)
        failed_batch.meta_info["feedback_tool_name"] = tool_name
        failed_batch.meta_info["do_sample"] = False      # 一般不采样
        failed_batch.meta_info["validate"] = False       

        return failed_batch
    
    # def _build_feedback_prompts_from_failed_batch_by_group(
    #     self,
    #     failed_batch: DataProto,
    #     **kwargs,
    # ) -> DataProto:
    ...
    #     - non_tensor_batch["raw_prompt"]
    #     - non_tensor_batch["tools_kwargs"]
    #     每个 entry 代表「一组 solutions」：
    #     question: 某个 failed prompt 的题目
    #     solutions: 这个 prompt 下被分到同一组的若干 rollout 文本
    #     然后可以直接丢给 _preprocess_prompt_to_async_rollout_requests。
    #     """
    ...
    #     # === 3. 决定「怎么分组」===
    #     meta = failed_batch.meta_info or {}
    #     group_ids = meta.get("feedback_group_ids", None)
    #     group_prompt_indices = meta.get("feedback_group_prompt_indices", None)

    #     groups: dict[int, dict] = {}

    #     if group_ids is not None:
    #         # 有你在 trainer 里预先算好的分组
    #         if len(group_ids) != num_rows:
    #             raise ValueError(
    #                 f"feedback_group_ids length {len(group_ids)} != failed_batch size {num_rows}"
    #             )
    ...
    #                 continue
    #             gid_to_rows[int(gid)].append(row_idx)

    #         for gid, rows in gid_to_rows.items():
    #             # 按你的设计：同一个 group 里的 question 相同，随便取一条即可
    #             first_row = rows[0]
    #             question = prompt_texts[first_row]
    #             solutions = [rollout_texts[r] for r in rows]
    ...
    #                 "rows": rows,
    #                 "prompt_idx": (
    #                     group_prompt_indices[gid]
    #                     if group_prompt_indices is not None
    #                     and gid < len(group_prompt_indices)
    #                     else None
    #                 ),
    #             }
    #     else:
    #         # 没有分组信息，就退回到「一条 rollout 就是一组，solutions=[solution]」
    #         for row_idx in range(num_rows):
    #             groups[row_idx] = {
    #                 "question": prompt_texts[row_idx],
    #                 "solutions": [rollout_texts[row_idx]],
    #                 "rows": [row_idx],
    #                 "prompt_idx": None,
    #             }

    #     # === 4. 为每个 group 构造 raw_prompt & tools_kwargs ===
    #     raw_prompts: list[list] = []
    #     tools_kwargs_list: list[dict] = []
    #     # 可选：为后续 merge 提供一份 group 的 meta
    #     feedback_group_keys: list[dict] = []

    #     # 这里按 group_id 排序，只是为了 deterministic
    ...
    #         # 4.1 构造 content（包含 question + 多个 solutions）
    #         tool_call_str = self._format_feedback_tool_call_by_group(
    #             tool_name=tool_name,
    #             question=question,
    #             solutions=solutions,   # ★ 这里传的是 list[str]
    #         )

    #         # 4.2 用和 _async_rollout_a_request 一样的逻辑解析 tool_call
    #         normed_content = tool_call_str
    #         parsed_tool_calls: list[OpenAIFunctionToolCall] = []
    ...
    #         # 4.3 构造一条 assistant message（这条 message 代表一个「分组请求」）
    #         assistant_msg = MessageCls(
    #             role="assistant",
    #             content=normed_content,
    #         )
    ...
    #         # 4.4 tools_kwargs：告诉 rollout 有哪些工具及其 kwargs
    #         tools_kwargs_list.append(
    #             {
    #                 tool_name: {
    #                     "execute_kwargs": {},
    ...
    #     # === 5. 把构造好的 raw_prompt / tools_kwargs / group_keys 塞回 failed_batch ===
    #     failed_batch.non_tensor_batch = dict(failed_batch.non_tensor_batch)
    #     failed_batch.non_tensor_batch["raw_prompt"] = np.array(raw_prompts, dtype=object)
    #     failed_batch.non_tensor_batch["tools_kwargs"] = np.array(tools_kwargs_list, dtype=object)
    #     # 可选：给之后 merge 用（可以不需要 tensor 化）
    #     failed_batch.non_tensor_batch["feedback_group_keys"] = np.array(feedback_group_keys, dtype=object)
    #     print("failed_batch.non_tensor_batch_raw_prompt:", failed_batch.non_tensor_batch["raw_prompt"])
    #     print("failed_batch.non_tensor_batch_tools_kwargs:", failed_batch.non_tensor_batch["tools_kwargs"])
    #     print("failed_batch.non_tensor_batch_feedback_group_keys:", failed_batch.non_tensor_batch["feedback_group_keys"])

    #     # meta_info 打标记
    #     failed_batch.meta_info = dict(failed_batch.meta_info)
    #     failed_batch.meta_info["feedback_tool_name"] = tool_name
    #     failed_batch.meta_info["feedback_num_groups"] = len(groups)
    #     failed_batch.meta_info["do_sample"] = False
    #     failed_batch.meta_info["validate"] = False
    ...
    def _build_feedback_prompts_from_failed_batch_by_group(
        ...
        **kwargs,
        ...
        """
        基于 _collect_all_negative_rollout_prompts_by_group 得到的 failed_batch，
        构造专门给 feedback-tool 用的 prompts：
        - non_tensor_batch["raw_prompt"]
        - non_tensor_batch["tools_kwargs"]
        ...
        # === 1. 取出 prompt_text / rollout_text ===
        nt = getattr(failed_batch, "non_tensor_batch", {}) or {}

        prompt_text_field = nt.get("prompt_text")
        rollout_text_field = nt.get("rollout_text")
        ...
        # 3.1 优先从 non_tensor_batch 里拿“按行对齐”的 group_ids（多卡安全）
        group_ids = None
        group_ids_field = nt.get("feedback_group_ids", None)
        if group_ids_field is not None:
            group_ids = (
                group_ids_field.tolist()
                if isinstance(group_ids_field, np.ndarray)
                else list(group_ids_field)
            )
            if len(group_ids) != num_rows:
                print(
                    f"[build_by_group] WARN: non_tensor feedback_group_ids length "
                    f"{len(group_ids)} != failed_batch size {num_rows}, ignore it.",
                    flush=True,
                )
                group_ids = None

        # 3.2 兼容老逻辑：没有 non_tensor 的话，再尝试从 meta_info 拿
        meta = failed_batch.meta_info or {}
        if group_ids is None:
            legacy_group_ids = meta.get("feedback_group_ids", None)
            if legacy_group_ids is not None:
                legacy_group_ids = list(legacy_group_ids)
                if len(legacy_group_ids) == num_rows:
                    group_ids = legacy_group_ids
                else:
                    print(
                        f"[build_by_group] WARN: meta feedback_group_ids length "
                        f"{len(legacy_group_ids)} != failed_batch size {num_rows}, "
                        f"fall back to 1-sample-per-group.",
                        flush=True,
                    )
                    group_ids = None  # 下面会退化为「一条 rollout 一组」

        # 3.3 group -> prompt_idx 的映射（全局级别，可以放 meta_info，不要求和 num_rows 对齐）
        group_prompt_indices = meta.get("feedback_group_prompt_indices", None)
        if group_prompt_indices is not None:
            group_prompt_indices = list(group_prompt_indices)
        ...
        for gid in sorted(groups.keys()):
            ...
            if self._function_call_parser and self._function_call_parser.has_tool_call(
                tool_call_str
            ):
                try:
        ...
        failed_batch.non_tensor_batch = dict(failed_batch.non_tensor_batch)
        failed_batch.non_tensor_batch["raw_prompt"] = np.array(raw_prompts, dtype=object)
        failed_batch.non_tensor_batch["tools_kwargs"] = np.array(
            tools_kwargs_list, dtype=object
        )
        # 给之后 merge 用（可以不需要 tensor 化，但这里沿用 DataProto 的 non_tensor 机制）
        failed_batch.non_tensor_batch["feedback_group_keys"] = np.array(
            feedback_group_keys, dtype=object
        )
    ...
    def _build_feedback_prompts_merged(
        self,
        feedback_batch: DataProto,
        **kwargs,
    ) -> DataProto:
        """
        在第一阶段已经得到 group-level feedback 之后，
        构造第二阶段「合并所有 group-feedback」用的 prompts：

        输出：
        - non_tensor_batch["raw_prompt"]：每一条是一个 prompt（题目），内容是
        question + feedbacks(list)，通过 tool_call 形式封装好。
        - non_tensor_batch["tools_kwargs"]
        - non_tensor_batch["merged_feedback_prompt_keys"]：记录每条合并请求对应哪个 prompt_idx、
        涉及哪些 group_id，方便之后对齐。
        """

        import numpy as np
        from collections import defaultdict

        # === 1. 取出必要的 non-tensor 字段 ===
        nt = feedback_batch.non_tensor_batch

        prompt_text_field = nt.get("prompt_text")
        feedback_text_field = nt.get("feedback_text")
        group_keys_arr = nt.get("feedback_group_keys")

        if group_keys_arr is None or feedback_text_field is None:
            raise ValueError(
                "_build_feedback_prompts_merged expects "
                "`feedback_group_keys` and `feedback_text` in non_tensor_batch."
            )

        # prompt_text 主要用来拿 question，如果拿不到就 fallback 成空字符串
        if prompt_text_field is not None:
            prompt_texts = (
                prompt_text_field.tolist()
                if isinstance(prompt_text_field, np.ndarray)
                else list(prompt_text_field)
        ...
        if len(group_keys) != len(feedback_texts):
            raise ValueError(
                f"_build_feedback_prompts_merged: feedback_group_keys length "
                f"{len(group_keys)} != feedback_text length {len(feedback_texts)}"
            )
        ...
        # === 2. 确定要调用哪个 merge-tool ===
        # 可以通过 kwargs 强制指定，否则读 config.multi_turn.feedback_merge_tool_name，
        # 再不行就用 fallback 名字。
        tool_name = (
            kwargs.get("tool_name")
            or getattr(self.config.multi_turn, "feedback_merge_tool_name", None)
            or getattr(self.config.multi_turn, "feedback_tool_name", None)  # 复用也行
            or "merge_feedbacks"
        )

        # === 3. 先按 prompt_idx 聚合所有 group-feedback ===
        # prompt_key -> list(feedback_text)
        prompt_to_texts: dict[int, list[str]] = defaultdict(list)
        # prompt_key -> 任意一个 group 对应的 question row，用来拿 question 文本
        prompt_to_rep_row: dict[int, int] = {}
        # prompt_key -> 涉及的 group_id 列表，方便 debug / 对齐
        prompt_to_group_ids: dict[int, list[int]] = defaultdict(list)
        ...
        for row_idx, key in enumerate(group_keys):
            ...
            if prompt_idx is None:
                # 没有 prompt_idx 的情况，就拿 row_idx 当 key（基本等价于「一题一 group」）
                prompt_key = int(row_idx)
            else:
                prompt_key = int(prompt_idx)

            prompt_to_texts[prompt_key].append(feedback_texts[row_idx])
            prompt_to_group_ids[prompt_key].append(key.get("group_id"))
            if prompt_key not in prompt_to_rep_row:
                prompt_to_rep_row[prompt_key] = row_idx

        if not prompt_to_texts:
            # 理论上不会走到这里
            return feedback_batch

        # === 4. 为每个 prompt 构造 raw_prompt & tools_kwargs ===
        raw_prompts: list[list] = []
        tools_kwargs_list: list[dict] = []
        merged_feedback_prompt_keys: list[dict] = []

        # 为了 deterministic，按 prompt_key 排序
        for prompt_key in sorted(prompt_to_texts.keys()):
            rep_row = prompt_to_rep_row[prompt_key]
            feedbacks_for_prompt = prompt_to_texts[prompt_key]

            # question 文本：如果有 prompt_texts，就根据 rep_row 拿；否则空串
            if prompt_texts is not None and rep_row < len(prompt_texts):
                question = prompt_texts[rep_row]
            else:
                question = ""

            # 4.1 构造 content（包含 question + 多个 feedbacks）
            tool_call_str = self._format_feedback_merged_by_group(
                tool_name=tool_name,
                question=question,
                feedbacks=feedbacks_for_prompt,
            )

            # 4.2 和 build_by_group 一样的 function-call 解析流程
            normed_content = tool_call_str
            parsed_tool_calls: list[OpenAIFunctionToolCall] = []
            ...
            # 4.3 构造一条 assistant message（这条 message 代表「合并这一题所有 group-feedback」）
            assistant_msg = MessageCls(
                role="assistant",
                content=normed_content,
            )
            ...
            merged_feedback_prompt_keys.append(
                {
                    "prompt_key": prompt_key,
                    "rep_row": rep_row,
                    "group_ids": prompt_to_group_ids[prompt_key],
                }
            )

        # === 5. 把构造好的 raw_prompt / tools_kwargs / merged_keys 塞回 feedback_batch ===
        feedback_batch.non_tensor_batch = dict(feedback_batch.non_tensor_batch)
        feedback_batch.non_tensor_batch["raw_prompt"] = np.array(raw_prompts, dtype=object)
        feedback_batch.non_tensor_batch["tools_kwargs"] = np.array(tools_kwargs_list, dtype=object)
        feedback_batch.non_tensor_batch["merged_feedback_prompt_keys"] = np.array(
            merged_feedback_prompt_keys, dtype=object
        )

        # meta_info 打标记（区分这是 merge 阶段的请求）
        feedback_batch.meta_info = dict(feedback_batch.meta_info)
        feedback_batch.meta_info["feedback_merge_tool_name"] = tool_name
        # feedback_batch.meta_info["feedback_merge_prompt_count"] = len(merged_feedback_prompt_keys)
        feedback_batch.meta_info["merge"] = True
        feedback_batch.meta_info["do_sample"] = False
        feedback_batch.meta_info["validate"] = True

        return feedback_batch


    async def _async_tool_only_rollout_a_request(
        self,
        req: AsyncRolloutRequest,
        do_sample: bool = True,
        is_validate: bool = False,
        **kwargs,
        ...
        """只执行工具调用，不做模型生成和互动。
        保持与 _async_rollout_a_request 相同的接口和返回类型。
        """
        assert self._tp_rank == 0, "only the master process can call this function"

        _req = deepcopy(req)
        finish_reason_type = FinishReasonTypeEnum.STOP  # 统一当作正常结束
        user_turn_rewards: list[float] = []

        # ------- 1. 预期状态检查 -------
        # 这个函数假定 request 已经处于 TOOL_CALLING 状态，
        # 且最后一条 message 中带有 tool_calls
        if _req.state == AsyncRolloutRequestStateEnum.PENDING:
            await self._handle_pending_state(_req)
            _req.state = AsyncRolloutRequestStateEnum.TOOL_CALLING
        ...
        if _req.state != AsyncRolloutRequestStateEnum.TOOL_CALLING:
            raise ValueError(
                f"_async_tool_only_rollout_a_request expects state=TOOL_CALLING, "
                f"but got {_req.state}"
            )

        if not _req.messages or _req.messages[-1].tool_calls is None:
            raise ValueError(
                f"Last message has no tool_calls: {_req.messages[-1] if _req.messages else 'EMPTY'}"
            )

        # ------- 2. 执行工具调用（原 TOOL_CALLING 分支的逻辑） -------
        parsed_tool_calls = _req.messages[-1].tool_calls

        # 如果跳过 tokenizer 初始化，则不在 messages 里保留 tool_calls
        if self.config.skip_tokenizer_init:
            _req.messages[-1].tool_calls = None
        ...
        # tool_call_results: List[(resp, reward, metrics)]

        # 把 tools 的返回值作为 tool response msg 加入对话
        _req.add_tool_response_messages(
        ...
        # 工具调用结束后，我们这里直接认为整个 request 完成
        _req.state = AsyncRolloutRequestStateEnum.COMPLETED

        # ------- 3. 计算工具 reward + release（原逻辑拷贝） -------
        async def calc_reward_and_release_fn(name: str, tool: BaseTool):
        ...
        # 将工具 reward 与 user_turn_rewards（这里一般为空列表）合并
        all_rewards = {**tool_reward_scores, **{"user_turn_rewards": user_turn_rewards}}

        # ------- 4. finalize：让 _req 进入“成品”状态 -------
        _req.finalize(self.processing_class, all_rewards, finish_reason_type)

        # ------- 5. 是否需要 log_probs？ -------
        # 如果你在 tool-only 场景下根本不需要 log_probs，建议直接不算：
        #   - 要么保证 self.config.calculate_log_probs = False
        #   - 要么这里返回空的张量，避免后续逻辑索引时报错
        if self.config.calculate_log_probs:
            # 这里不再调用 engine，只给出空张量或你需要的占位符
            # 注意：如果后续会用到这些值的实际内容，就需要自行定义逻辑
            device = _req.input_ids.device
            _req.output_token_ids = torch.empty(0, dtype=torch.long, device=device)
            _req.rollout_log_probs = torch.empty(0, dtype=torch.float, device=device)
    ...
    @GPUMemoryLogger(role="sglang rollout", logger=logger)
    @torch.no_grad()
    def _generate_feedback_per_rollout(self, prompts: DataProto, **kwargs)  -> DataProto:
        # 1. 先按你原来的逻辑，构造 tool-only 的请求
        do_sample = prompts.meta_info.get("do_sample", True)
        is_validate = prompts.meta_info.get("validate", False)
        tgt_device = prompts.batch["input_ids"].device

        meta = prompts.meta_info or {}
        is_merge = bool(meta.get("merge", False))
        if is_merge:
                # 第二阶段：合并多个 group-feedback
                prompts = self._build_feedback_prompts_merged(
                    prompts,
                    tool_name="calc_gsm8k_reward",   # ★ 用同一个工具名
                )
        else:
            # 第一阶段：一题多解 -> group-level feedback
            prompts = self._build_feedback_prompts_from_failed_batch_by_group(
                prompts,
                tool_name="calc_gsm8k_reward",   # ★ 一样
            )
        ...
        if self._tp_rank == 0:
            ...
            if is_validate:
                ...
                output_req_list = loop.run_until_complete(
                    asyncio.gather(
                        *[
                            self._async_tool_only_rollout_a_request(
                ...
                # 这里保持和 _req_level_generate_sequences 一样的 abort 逻辑
                total_requests = len(req_list)
                target_completion = int(
                    total_requests * (1 - self.config.get("over_sample_rate", 0.0))
                )
                ...
                async def rollout_a_request_with_cancellation_handler(req):
                    try:
                        result = await self._async_tool_only_rollout_a_request(
                            req, do_sample, is_validate, **kwargs
                        )
                        return result
                    except asyncio.CancelledError:
                        logger.info(
                            f"Request {req.request_id} was cancelled, creating padding"
                        )
                        aborted_requests.append(req.request_id)
                        return self._create_padding_request(req)

                async def run_with_cancellation():
                    nonlocal all_tasks, completed_count
                    all_tasks = [
                        asyncio.create_task(
                            rollout_a_request_with_cancellation_handler(req)
                        )
                        for req in req_list
                    ]
                    try:
        ...
        # ====== 2. 从 req 里收集我们真正需要的东西：feedback_text / messages / reward_scores / request_ids ======
        messages = []
        reward_scores = []
        request_ids = []
        multi_modal_inputs = []
        ...
        for req in sorted_output_req_list:
            ...
            #     tool_feedback = "no feedback provided"
            #     # print("content:",content)
            #     # print("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
            #     # 1）标准 OpenAI 风格的 tool message: role == "tool"
            #     if role == "tool" and content is not None:
            #         if isinstance(content, str):
            #             tool_feedback = content
            #         elif isinstance(content, list):
            #             # 兼容 content 是 list[{"type": "text", "text": "..."}] 的情况
            #             parts = []
            #             for p in content:
            #                 if isinstance(p, dict):
            #                     parts.append(p.get("text", ""))
            #             tool_feedback = "".join(parts)
            #         break
            tool_feedback = "no feedback provided"
            # 从最后一条往前找 tool 消息
            for msg in reversed(req.messages):
                ...
                if role == "tool" and content:
                    ...
                    if isinstance(content, str):
                        ...
                        tool_feedback = "".join(parts)
                    break
            feedback_texts.append(tool_feedback)
        # print("Generated messages:", messages)
        # print("Generated feedback_texts:", feedback_texts)
        num_reqs = len(sorted_output_req_list)

        # ====== 3. 关键改动：构造一个统一 shape 的 dummy batch ======
        # 我们不再用 req.prompt_ids / req.response_ids 真实长度，
        # 只造一个 seq_len = 1 的占位 tensor，保证所有 worker 形状完全一致。
        dummy_len = 1
        ...
        attention_mask = torch.zeros(
            (num_reqs, dummy_len), dtype=torch.long, device=tgt_device
        )
        position_ids = torch.zeros(
            (num_reqs, dummy_len), dtype=torch.long, device=tgt_device
        )
        prompts_ids = input_ids.clone()
        responses_ids = input_ids.clone()
        ...
        # 注意：feedback 这条链路我们根本不需要 log_probs，
        # 所以完全不往 batch 里塞 "rollout_log_probs" / "rollout_output_token_ids"，
        # 避免额外 shape 对齐问题。
        # if self.config.calculate_log_probs:
        #     ...
        ...
        non_tensor_batch = {
            ...
            "feedback_text": np.array(feedback_texts, dtype=object),
        }

        # === 把 prompts 里的一些 non-tensor 辅助信息一并带回 trainer ===

        # 1) 分组信息：给 _merge_failed_feedbacks_global 用
        orig_nt = getattr(prompts, "non_tensor_batch", {}) or {}
        if not is_merge and "feedback_group_keys" in orig_nt:
            
            non_tensor_batch["feedback_group_keys"] = orig_nt["feedback_group_keys"]
        ...
        feedback_batch = DataProto(batch=batch, non_tensor_batch=non_tensor_batch)
        # 带上原来的 meta_info（尤其是 repeat_times / failed_prompt_count）
        feedback_batch.meta_info = dict(prompts.meta_info)
        feedback_batch.meta_info.setdefault("timing", {})["gen_feedback"] = 0.0
    ...
    @GPUMemoryLogger(role="sglang rollout", logger=logger)
    @torch.no_grad()
    def generate_feedbacks(self, prompts: DataProto, **kwargs) -> DataProto:
        feedback_batch = self._generate_feedback_per_rollout(prompts, **kwargs)
        # return self._merge_feedbacks(feedback_batch)
        return feedback_batch
    

    @GPUMemoryLogger(role="sglang rollout", logger=logger)
    @torch.no_grad()
    def _merge_feedbacks(self, feedback_batch: DataProto) -> DataProto:
        """把 rollout 粒度的 feedback 压缩成 prompt 粒度：每个 prompt 随机选 1 条反馈。"""

        meta = getattr(feedback_batch, "meta_info", {}) or {}
        repeat_times = meta.get("repeat_times", None)
        failed_prompt_count = meta.get("failed_prompt_count", None)

        # 如果没有这些 meta 信息，就直接原样返回，避免 silent bug
        if repeat_times is None or failed_prompt_count is None:
            logger.warning(
                "Feedback batch meta_info missing 'repeat_times' or 'failed_prompt_count'; "
                "skip merging and return as-is."
            )
            return feedback_batch

        # 取当前 batch size（按任意一个 tensor 的第一维）
        batch_size = None
        ...
        if batch_size is None:
            # 极端情况：batch 里没有 tensor，直接返回
            return feedback_batch

        # 计算每个 prompt 的起止 indices，并在其中随机选一个
        selected_indices: list[int] = []
        for prompt_idx in range(int(failed_prompt_count)):
            start = prompt_idx * int(repeat_times)
            if start >= batch_size:
                break
            end = min(start + int(repeat_times), batch_size)
            if end <= start:
                continue
            # 在 [start, end) 中随机选一个
        ...
        if not selected_indices:
            logger.warning("No indices selected in _merge_feedbacks; returning original feedback_batch.")
            return feedback_batch

        # 用 DataProto 自己的 select_idxs 做子集选择（你之前在 _collect_all_negative_rollout_prompts 里就是这么用的）
        merged = feedback_batch.select_idxs(selected_indices)

        # 更新 meta_info：现在 repeat_times 实际上是 1 了，failed_prompt_count = 样本数
        merged.meta_info = dict(meta)
        merged.meta_info["repeat_times"] = 1
        merged.meta_info["failed_prompt_count"] = len(selected_indices)
