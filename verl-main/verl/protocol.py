...
def list_of_dict_to_dict_of_list(list_of_dict: list[dict]):
    ...
    all_keys = set(key for data in list_of_dict for key in data.keys())
...
class DataProto:
    ...
    def concat(data: list["DataProto"]) -> "DataProto":
        ...
        if data:
            ...
            if all_metrics:
                keysets = [tuple(sorted(m.keys())) for m in all_metrics]
                base = set(all_metrics[0].keys())
                for i, m in enumerate(all_metrics):
                    ks = set(m.keys())
                    if ks != base:
                        print("[metrics mismatch] i=", i,
