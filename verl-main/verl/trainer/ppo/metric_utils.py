...
def compute_data_metrics(batch: Optional[DataProto], use_critic: bool = True) -> dict[str, Any]:
    ...
    # advantages / returns 可能在 DPO-only 或 mixed concat 后缺失
    ...
    # response_mask 可能缺失：按 attention_mask + responses 自动推
    max_response_length = batch.batch["responses"].shape[-1]
    ...
    # 注意：masked_select 可能拿到空 tensor（例如全 aborted）
    valid_adv = torch.masked_select(advantages, response_mask)
    valid_returns = torch.masked_select(returns, response_mask)
