...
def _safe_stats(x: torch.Tensor):
    ...
    return torch.mean(x).detach().item(), torch.max(x).detach().item(), torch.min(x).detach().item()

def compute_data_metrics(batch: Optional[DataProto], use_critic: bool = True) -> dict[str, Any]:
    # ---------- 0) allow None / empty ----------
    if batch is None:
        return {"critic/empty_batch": 1}
    try:
        if len(batch) == 0:
            return {"critic/empty_batch": 1}
    except Exception:
        # Some DataProto-like objects may not implement len().
        pass

    # ---------- 1) required-ish tensors with fallbacks ----------
    tl_scores = batch.batch.get("token_level_scores", None)
    if tl_scores is None:
        return {"critic/empty_batch": 1}

    tl_rewards = batch.batch.get("token_level_rewards", tl_scores)

    # FCO-only or mixed batches may not carry advantages / returns.
    advantages = batch.batch.get("advantages", torch.zeros_like(tl_rewards))
    returns = batch.batch.get("returns", torch.zeros_like(tl_rewards))

    # Fall back to attention_mask + responses if response_mask is absent.
    max_response_length = batch.batch["responses"].shape[-1]
    ...
    # ---------- 2) sequence stats (exclude aborted) ----------
    sequence_score = tl_scores.sum(-1)
    sequence_reward = tl_rewards.sum(-1)
    ...
    score_mean, score_max, score_min = _safe_stats(non_aborted_sequence_score)
    reward_mean, reward_max, reward_min = _safe_stats(non_aborted_sequence_reward)

    # ---------- 3) token-level stats on valid response positions ----------
    # masked_select may return an empty tensor, e.g. when all samples are aborted.
    valid_adv = torch.masked_select(advantages, response_mask)
    valid_returns = torch.masked_select(returns, response_mask)
    ...
    metrics = {
        ...
        # response length
        "response_length/mean": torch.mean(response_length).detach().item() if response_length.numel() else float("nan"),
        "response_length/max": torch.max(response_length).detach().item() if response_length.numel() else float("nan"),
        "response_length/min": torch.min(response_length).detach().item() if response_length.numel() else float("nan"),
        "response_length/clip_ratio": (
            torch.mean(torch.eq(response_length, max_response_length).float()).detach().item()
            if response_length.numel()
            else float("nan")
        ),
        # aborted ratio
        "response/aborted_ratio": torch.mean(aborted_mask.float()).detach().item() if aborted_mask.numel() else 1.0,
        # prompt length
        "prompt_length/mean": torch.mean(prompt_length).detach().item() if prompt_length.numel() else float("nan"),
        "prompt_length/max": torch.max(prompt_length).detach().item() if prompt_length.numel() else float("nan"),
        "prompt_length/min": torch.min(prompt_length).detach().item() if prompt_length.numel() else float("nan"),
        "prompt_length/clip_ratio": (
            torch.mean(torch.eq(prompt_length, max_prompt_length).float()).detach().item()
            if prompt_length.numel()
            else float("nan")
        ),
    }

    # ---------- 4) non-aborted-only response length stats (no longer raise) ----------
    non_aborted_response_length = response_length[non_aborted_mask]
    if non_aborted_response_length.numel() > 0:
        metrics.update(
            {
                ...
                "response_length_non_aborted/clip_ratio": torch.mean(
        ...
        # If all samples are aborted, report NaN instead of raising.
        metrics.update(
            {
                ...
                "response_length_non_aborted/clip_ratio": float("nan"),
            }
        )

    # ---------- 5) critic metrics: only if available ----------
    if use_critic and ("values" in batch.batch):
        values = batch.batch["values"]
        valid_values = torch.masked_select(values, response_mask)
        val_mean, val_max, val_min = _safe_stats(valid_values)

        # Guard explained variance against empty tensors.
        if valid_returns.numel() > 0 and valid_values.numel() > 0:
            return_diff_var = torch.var(valid_returns - valid_values)
            return_var = torch.var(valid_returns)
            vf_ev = (1.0 - return_diff_var / (return_var + 1e-5)).detach().item()
        else:
            vf_ev = float("nan")
        ...
        metrics.update(
            {
                ...
                "critic/vf_explained_var": vf_ev,
            }
        )

    # multi-turn conversation
    if hasattr(batch, "non_tensor_batch") and ("__num_turns__" in batch.non_tensor_batch):
        num_turns = batch.non_tensor_batch["__num_turns__"]
        metrics["num_turns/min"] = num_turns.min()
        metrics["num_turns/max"] = num_turns.max()
        metrics["num_turns/mean"] = num_turns.mean()

    if hasattr(batch, "non_tensor_batch") and ("tool_call_counts" in batch.non_tensor_batch):
        tool_call_counts = batch.non_tensor_batch["tool_call_counts"]
        metrics["tool_call_counts/min"] = tool_call_counts.min()
        metrics["tool_call_counts/max"] = tool_call_counts.max()
        metrics["tool_call_counts/mean"] = tool_call_counts.mean()
