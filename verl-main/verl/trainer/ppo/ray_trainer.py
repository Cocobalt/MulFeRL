...
class RayPPOTrainer:
    ...
    def init_workers(self):
        ...
        if self.config.actor_rollout_ref.rollout.mode == "async":
            raise NotImplementedError(
                "Async agent-loop rollout code is omitted from this anonymous MulFeRL release. "
                "Use actor_rollout_ref.rollout.mode=sync."
            )
    ...
    def _collect_all_negative_rollout_prompts_by_group(
        ...
        prompt_texts_per_prompt = [
            (e or {}).get("question_raw") or (e or {}).get("question") or ""
            for e in failed_anchor_extra.tolist()
        ]
        ...
        feedback_group_size = int(self.config.algorithm.get("feedback_group_size", 2) or 2)
    ...
    def _build_regen_batch(self, failed_prompts: DataProto, failed_feedback_batch: DataProto) -> DataProto:
        ...
        if extra_infos is not None:
            ...
            question_texts = [
                (ei or {}).get("question") or (ei or {}).get("question_raw") or ""
    ...
    def _build_fco_pair_batch(
        ...
        """
        Build an FCO training batch from aligned chosen/rejected indices.

        Output layout (2*K rows), interleaved for stable micro-batching:
          - rows: [c0, r0, c1, r1, ..., c{K-1}, r{K-1}]
          - batch.batch["fco_is_chosen"]: bool pair flag, True for chosen
          - batch.batch["fco_pair_id"]: pair id in [0..K-1], repeated twice per pair
          - batch.meta_info["loss_type"] = "fco"
        """
        import torch
        ...
        if chosen_idx.numel() != rejected_idx.numel():
            raise ValueError(f"[FCO] chosen/rejected size mismatch: {chosen_idx.numel()} vs {rejected_idx.numel()}")
        ...
        fco_dp = batch.select_idxs(interleave_idx)

        K = int(chosen_idx.numel())
        device = next(iter(fco_dp.batch.values())).device

        # is_chosen: [True, False, True, False, ...]
        fco_dp.batch["fco_is_chosen"] = (torch.arange(2 * K, device=device) % 2 == 0)

        # pair_id: [0, 0, 1, 1, 2, 2, ...]
        fco_dp.batch["fco_pair_id"] = torch.arange(K, device=device, dtype=torch.long).repeat_interleave(2)

        
        # Placeholder for mixed-loss actor: FCO rows do not use seq_level_rewards, but mixed routing
        # expects the tensor to exist for the whole batch.
        if "seq_level_rewards" not in fco_dp.batch:
            fco_dp.batch["seq_level_rewards"] = torch.zeros((2 * K,), device=device, dtype=torch.float32)
        fco_dp.meta_info = dict(getattr(fco_dp, "meta_info", {}) or {})
        fco_dp.meta_info["loss_type"] = "fco"
        fco_dp.meta_info["fco_pairs"] = K

        # refresh token count for flops/metrics
        self._refresh_global_token_num(fco_dp)

        metrics["feedback/fco_pair_cnt"] = K
        metrics["feedback/fco_sample_cnt"] = int(2 * K)
        return fco_dp


    def _prepare_grpo_and_fco_batches(
        self,
        batch: DataProto,
        n_rollout: int,
        ...
        - regen uid: look at regen's n rewards
            * all 1 -> FCO (regen as chosen vs base as rejected)
            * all 0 -> drop
            * mixed -> GRPO on regen only

        Returns:
            grpo_batch: DataProto for GRPO update (has advantages/returns)
            fco_batch : DataProto for FCO update (has fco_pair_id/fco_is_chosen fields)
        """
        import numpy as np
        import torch
        ...
        for uid, base_list in uid_to_base.items():
            ...
            if regen_pos.all().item():
                ...
                if k <= 0:
                    dropped_uid_cnt += 1
                    continue
                # Permutation-based FCO sampling: pair each solved rollout with
                # a randomly permuted failed rollout from the previous state.
                perm = torch.randperm(len(base_list), device=rm.device)[:k]
                chosen_idx_list.append(regen_t[:k])
                rejected_idx_list.append(base_t[perm])
                continue
        ...
        metrics["feedback/fco_uid_cnt"] = int(fco_uid_cnt)
        ...
        # --------- 4) build FCO batch ---------
        fco_batch = self._build_fco_pair_batch(
            batch=batch,
    ...
    def _collect_multiturn_regen_batches(
        ...
        for turn in range(1, max_turn + 1):
            ...
            with marked_timer(f"regen_turn{turn}_gen", timing_raw, color="red"):
                ...
                out_nt.update(dict(getattr(regen_out, "non_tensor_batch", {}) or {}))
            ...
            if r_all1.any().item():
                ...
                rejected_rows = []
                for prompt_idx in sel_base_prompt_idx.tolist():
                    perm = torch.randperm(n_rollout, device=device)
                    rejected_rows.append(prompt_idx * n_rollout + perm)
                flat_rejected = torch.cat(rejected_rows, dim=0)
        ...
        if chosen_all is not None and rejected_all is not None and len(chosen_all) > 0:
            ...
            fco_batch_pre = self._build_fco_pair_batch(pool, chosen_idx, rejected_idx, metrics)

        return grpo_batch_pre, fco_batch_pre, metrics
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    grpo_pre, fco_pre, regen_metrics = self._collect_multiturn_regen_batches(
                        base_batch=batch,
                    ...
                    has_fco = (fco_pre is not None and len(fco_pre) > 0)
                    ...
                    if has_fco:
                        actor_batch = fco_pre
                        fco_size = len(fco_pre)

                    if has_grpo:
                        device = next(iter(grpo_pre.batch.values())).device
                        grpo_pre.batch["fco_pair_id"] = torch.full((len(grpo_pre),), -1, dtype=torch.long, device=device)
                        grpo_pre.batch["fco_is_chosen"] = torch.zeros((len(grpo_pre),), dtype=torch.bool, device=device)
                    ...
                    if actor_batch is None or len(actor_batch) == 0:
                        ...
                        actor_batch.meta_info["_fco_size"] = int(fco_size)
                        actor_batch.meta_info["_grpo_size"] = int(grpo_size)

                        if has_fco and has_grpo:
                            actor_batch.meta_info["loss_type"] = "mixed"
                        elif has_fco:
                            actor_batch.meta_info["loss_type"] = "fco"
                        else:
                            actor_batch.meta_info["loss_type"] = "grpo"

                        if has_fco:
                            actor_batch.meta_info["fco_tau"] = float(
                                self.config.algorithm.get("fco_tau", 0.1)
                            )
                            actor_batch.meta_info["fco_lambda"] = float(
                                self.config.algorithm.get("fco_lambda", 1.0)
                            )
                            actor_batch.meta_info["fco_margin"] = float(self.config.algorithm.get("fco_margin", 0.0))
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        fco_size = int(batch.meta_info.get("_fco_size", 0) or 0)
                        ...
                        if fco_size > 0:
                            fco_idx = torch.arange(fco_size, device=device, dtype=torch.long)
                            fco_part = batch.select_idxs(fco_idx)

                        if grpo_size > 0:
                            grpo_idx = torch.arange(fco_size, fco_size + grpo_size, device=device, dtype=torch.long)
                            ...
                            grpo_part.batch["fco_pair_id"] = torch.full((len(grpo_part),), -1, dtype=torch.long, device=device)
                            grpo_part.batch["fco_is_chosen"] = torch.zeros((len(grpo_part),), dtype=torch.bool, device=device)

                        batch_for_critic = grpo_part

                        # rebuild actor batch
                        actor_batch = None
                        if fco_part is not None and len(fco_part) > 0:
                            actor_batch = fco_part
