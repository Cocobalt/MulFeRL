...
def _get_question_raw(ei):
    # extra_info may be a dict, a JSON string, or None.
    if ei is None:
        return ""
    if isinstance(ei, dict):
        return ei.get("question_raw", "") or ""
    if isinstance(ei, str):
        try:
            obj = json.loads(ei)
            if isinstance(obj, dict):
                return obj.get("question_raw", "") or ""
        except Exception:
            pass
    return ""
...
class SGLangRollout(BaseRollout):
    ...
    def _req_level_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if prompt_ids.shape[-1] > self.config.prompt_length:
            prompt_ids = prompt_ids[:, -self.config.prompt_length :]
        if prompt_ids.shape[-1] < self.config.prompt_length:
        ...
        if prompt_attention_mask.shape[-1] > self.config.prompt_length:
            prompt_attention_mask = prompt_attention_mask[:, -self.config.prompt_length :]
        if prompt_attention_mask.shape[-1] < self.config.prompt_length:
        ...
        if response_attention_mask.shape[-1] > self.config.response_length:
            response_attention_mask = response_attention_mask[:, : self.config.response_length]
        if response_attention_mask.shape[-1] < self.config.response_length:
        ...
        if prompt_position_ids.shape[-1] > self.config.prompt_length:
            prompt_position_ids = prompt_position_ids[..., -self.config.prompt_length :]
        if prompt_position_ids.shape[-1] < self.config.prompt_length:
        ...
        if response_position_ids.shape[-1] > self.config.response_length:
            response_position_ids = response_position_ids[..., : self.config.response_length]
        if response_position_ids.shape[-1] < self.config.response_length:
        ...
        if response_loss_mask.shape[1] > self.config.response_length:
            response_loss_mask = response_loss_mask[:, : self.config.response_length]
        if response_loss_mask.shape[1] < self.config.response_length:
            response_loss_mask = pad_sequence_to_length(
                response_loss_mask, self.config.response_length, 0
            )

        # ===================== log_probs & rollout_output_token_ids =====================
        if self.config.calculate_log_probs:
        ...
        empty_mask = (attention_mask.sum(dim=-1) == 0)  # [B]
        if empty_mask.any():

            attention_mask[empty_mask, 0] = 1
            if position_ids.dim() == 2:

                position_ids[empty_mask, 0] = 0
    ...
    def _build_feedback_prompts_from_failed_batch_by_group(
        ...
        **kwargs,
        ...
        # Read prompt_text and rollout_text from the failed rollout batch.
        nt = getattr(failed_batch, "non_tensor_batch", {}) or {}

        prompt_text_field = nt.get("prompt_text")
        # extra_info_field = nt.get("extra_info")
        # prompt_text_field = [_get_question_raw(ei) for ei in extra_info_field]
        ...
        for gid in sorted(groups.keys()):
            g = groups[gid]
            rep_rows.append(g["rows"][0])        
            group_questions.append(g["question"])  

        device = next(iter(failed_batch.batch.values())).device if len(failed_batch.batch) > 0 else "cpu"
        rep_idx_tensor = torch.tensor(rep_rows, dtype=torch.long, device=device)

        feedback_prompts = failed_batch.select_idxs(rep_idx_tensor)

        feedback_prompts.non_tensor_batch = dict(getattr(feedback_prompts, "non_tensor_batch", {}) or {})
        feedback_prompts.non_tensor_batch["raw_prompt"] = np.array(raw_prompts, dtype=object)
        feedback_prompts.non_tensor_batch["tools_kwargs"] = np.array(tools_kwargs_list, dtype=object)
        feedback_prompts.non_tensor_batch["feedback_group_keys"] = np.array(feedback_group_keys, dtype=object)
        feedback_prompts.non_tensor_batch["prompt_text"] = np.array(group_questions, dtype=object)

        feedback_prompts.meta_info = dict(getattr(feedback_prompts, "meta_info", {}) or {})
        feedback_prompts.meta_info["feedback_tool_name"] = tool_name
        feedback_prompts.meta_info["feedback_num_groups"] = len(groups)
        feedback_prompts.meta_info["do_sample"] = False
        feedback_prompts.meta_info["validate"] = False
    ...
    def _generate_feedback_per_rollout(self, prompts: DataProto, **kwargs)  -> DataProto:
        ...
        if is_merge:
                prompts = self._build_feedback_prompts_merged(
                    prompts,
                    tool_name=(
                        getattr(self.config.multi_turn, "feedback_merge_tool_name", None)
                        or getattr(self.config.multi_turn, "feedback_tool_name", None)
                        or "calc_mulferl_feedback"
                    ),
                )
            ...
            prompts = self._build_feedback_prompts_from_failed_batch_by_group(
                prompts,
                tool_name=getattr(self.config.multi_turn, "feedback_tool_name", None) or "calc_mulferl_feedback",
            )
        ...
        if (not is_merge) and ("feedback_group_keys" in orig_nt):
            non_tensor_batch["feedback_group_keys"] = _align_to_local(orig_nt["feedback_group_keys"], num_reqs)

        if "prompt_text" in orig_nt:
            non_tensor_batch["prompt_text"] = _align_to_local(orig_nt["prompt_text"], num_reqs)
