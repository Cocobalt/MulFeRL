...
def compute_fco_loss_from_micro(
    log_prob: torch.Tensor,              # (bs, resp_len), requires grad
    response_mask: torch.Tensor,         # (bs, resp_len), 0/1 or bool
    pair_id: torch.Tensor,               # (bs,), long; -1 for non-FCO rows
    is_chosen: torch.Tensor,             # (bs,), bool
    tau: float = 0.1,
    margin: float = 0.0,
    ref_log_prob=None,                  # (bs, resp_len) optional
):
    """
    Compute the FCO logistic loss within one micro-batch.

    Notes:
      - Only rows with pair_id >= 0 are used.
      ...
      - Scores are length-normalized log ratios against the reference policy when ref_log_prob is available.
    ...
    Returns:
      ...
      num_pairs: int K
    """
    if pair_id is None or is_chosen is None:
        return None, None, 0
    ...
    pid = pair_id[mask].to(torch.long)
    is_c = is_chosen[mask].bool()
    lp_tok = log_prob[mask]
    rm = response_mask[mask].bool()

    lengths = rm.to(lp_tok.dtype).sum(dim=-1).clamp_min(1.0)
    seq_lp = (lp_tok * rm.to(lp_tok.dtype)).sum(dim=-1) / lengths

    if ref_log_prob is not None:
        ref_tok = ref_log_prob[mask]
        seq_ref = (ref_tok * rm.to(ref_tok.dtype)).sum(dim=-1) / lengths.to(ref_tok.dtype)
    else:
    ...
    loss = -F.logsigmoid(tau * (delta - margin)).mean()
...
class DataParallelPPOActor(BasePPOActor):
    ...
    def update_policy(self, data: DataProto):
        ...
        # ---- loss routing: "grpo"(default) | "fco" | "mixed" ----
        loss_type = data.meta_info.get("loss_type", "grpo")
        fco_tau = float(data.meta_info.get("fco_tau", 0.1))
        fco_lambda = float(data.meta_info.get("fco_lambda", 1.0))
        fco_margin = float(data.meta_info.get("fco_margin", 0.0))
        fco_loss_types = ("fco", "mixed")

        # FCO needs pair alignment inside micro-batches; dynamic bsz may reorder/split pairs.
        if loss_type in fco_loss_types:
            if self.config.use_dynamic_bsz:
                raise ValueError("FCO/mixed requires use_dynamic_bsz=False (pair alignment may break).")
            if (self.config.ppo_micro_batch_size_per_gpu % 2) != 0:
                raise ValueError(
                    "For FCO/mixed, ppo_micro_batch_size_per_gpu must be even to keep pairs in the same micro-batch."
                )
        ...
        if loss_type in fco_loss_types:
            pair_id_key = "fco_pair_id"
            is_chosen_key = "fco_is_chosen"
            select_keys += [pair_id_key, is_chosen_key]
        else:
            pair_id_key = "fco_pair_id"
            is_chosen_key = "fco_is_chosen"

        if self.config.use_kl_loss and ("ref_log_prob" in data.batch.keys()):
            # KL is only applied to GRPO rows (see below). For FCO, ref_log_prob is used in the score.
            select_keys.append("ref_log_prob")
        elif (loss_type in fco_loss_types) and ("ref_log_prob" in data.batch.keys()):
            # Use as the reference term in FCO if provided.
            select_keys.append("ref_log_prob")
        ...
        has_multi_modal_inputs = "multi_modal_inputs" in data.non_tensor_batch.keys()
        non_tensor_select_keys = ["multi_modal_inputs"] if has_multi_modal_inputs else []
        if has_multi_modal_inputs and loss_type in fco_loss_types:
            raise ValueError("FCO/mixed with multi_modal_inputs is not supported in this implementation.")
        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    # ===== GRPO/PPO loss (only for GRPO rows; in mixed, fco_pair_id < 0) =====
                    grpo_loss = None
                    pg_loss = None
                    pg_clipfrac = None
                    ppo_kl = None
                    ...
                    if loss_type in ("grpo", "mixed"):
                        if loss_type == "mixed":
                            grpo_rows = (model_inputs[pair_id_key] < 0)
                        else:
                        ...
                        if grpo_rows is None or torch.any(grpo_rows):
                            ...
                            # KL only for GRPO rows; FCO uses the reference in its score.
                    ...
                    # ===== FCO loss (only for FCO rows; pairs must be inside micro-batch) =====
                    fco_loss = None
                    fco_delta = None
                    fco_pairs = 0
                    if loss_type in fco_loss_types:
                        ref_lp = model_inputs.get("ref_log_prob", None)
                        fco_loss, fco_delta, fco_pairs = compute_fco_loss_from_micro(
                            log_prob=log_prob,
                            response_mask=response_mask,
                            pair_id=model_inputs[pair_id_key],
                            is_chosen=model_inputs[is_chosen_key],
                            tau=fco_tau,
                            margin=fco_margin,
                            ref_log_prob=ref_lp,
                        )
                        if fco_loss is not None:
                            fco_loss = fco_loss * fco_lambda
                        else:
                            if loss_type == "fco":
                                raise ValueError(
                                    "FCO loss has no complete pairs in this micro-batch. "
                    ...
                    if loss_type == "grpo":
                        ...
                        total_policy_loss = fco_loss
                    else:  # mixed
                        if grpo_loss is None and fco_loss is None:
                            continue
                        total_policy_loss = 0.0
                        if grpo_loss is not None:
                            total_policy_loss = total_policy_loss + grpo_loss
                        if fco_loss is not None:
                            total_policy_loss = total_policy_loss + fco_loss
                    ...
                    if fco_loss is not None:
                        micro_batch_metrics["fco/loss"] = fco_loss.detach().item() * loss_scale_factor
                        micro_batch_metrics["fco/pairs"] = float(fco_pairs)
                        if fco_delta is not None and fco_delta.numel() > 0:
                            micro_batch_metrics["fco/delta_mean"] = float(fco_delta.mean().item())
                            micro_batch_metrics["fco/delta_min"] = float(fco_delta.min().item())
                            micro_batch_metrics["fco/delta_max"] = float(fco_delta.max().item())
                    else:
                        micro_batch_metrics["fco/loss"] = 0.0
                        micro_batch_metrics["fco/pairs"] = 0.0
                        micro_batch_metrics["fco/delta_mean"] = 0.0
                        micro_batch_metrics["fco/delta_min"] = 0.0
                        micro_batch_metrics["fco/delta_max"] = 0.0
