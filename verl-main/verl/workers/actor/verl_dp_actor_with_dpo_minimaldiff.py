                        ...
                        micro_batch_metrics.update(
                            {
                                ...
                                "actor/pg_clipfrac": pg_clipfrac.detach().item() if pg_clipfrac is not None else 0.0,
                                "actor/ppo_kl": ppo_kl.detach().item() if ppo_kl is not None else 0.0,
                                "actor/pg_clipfrac_lower": pg_clipfrac_lower.detach().item() if pg_clipfrac_lower is not None else 0.0,
                            }
                        )
                    if dpo_loss is not None:
