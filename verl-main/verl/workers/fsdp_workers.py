...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def generate_sequences(self, prompts: DataProto):
        print("oooono! wrong generate_sequences called")
        ...
        with simple_timer("generate_sequences", timing_generate):
            print("calling rollout generate_sequences")
    ...
    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="rollout"))
    @DistProfiler.annotate(color="red", role="rollout_generate_feedbacks")
    def generate_feedbacks(self, prompts: DataProto):
        """
        Generate 'feedback' sequences for failed samples.

        设计思路：
        - 接口签名和 generate_sequences 一样，输入/输出都是 DataProto，
          这样 ray_trainer 里的 failed_batch_feedback 可以直接复用同一条链路。
        - 上层区别只是 trainer 调的是 generate_feedbacks，用来“重生成失败样本”。
        - 底层 rollout engine（sglang/hf）本身只认 prompts，不在乎这批是正常还是
          “失败样本”，因此这里直接调用 self.rollout.generate_sequences。
        """
        # 只在 rollout 角色下可用
        assert self._is_rollout
        ...
        # 在 hybrid engine 下，如果本 worker 同时是 actor，需要切到 rollout 模式
        if self._is_actor:  # For rollout only, we do not switch context for pure rollout worker.
            loop = get_event_loop()
            loop.run_until_complete(self.rollout_mode())
            log_gpu_memory_usage("After switch to rollout mode (feedbacks)", logger=logger)

        # 这里计时 key 用 "generate_feedbacks"，避免和 generate_sequences 搞混
        with simple_timer("generate_feedbacks", timing_generate):
            # 核心：底层仍然调用 rollout.generate_sequences
            output = self.rollout.generate_feedbacks(prompts=prompts)

        # 切回 trainer 模式
        if self._is_actor:
            loop.run_until_complete(self.trainer_mode())
            log_gpu_memory_usage("After switch to trainer mode (feedbacks)", logger=logger)

        # 和 generate_sequences 一样做 timing 聚合，只是读取的 key 不同
        timing_generate_topk_ratio, timing_generate_min, timing_generate_max = topk_reduce_ratio_min_max(
            timing_generate["generate_feedbacks"]
        )
        timing_generate = reduce_timing(timing_generate)
        timing_generate.update(
            {
                # 这里 meta_info 的 key 继续复用 generation_timing，方便下游统一处理
                "generation_timing/max": timing_generate_max,
                "generation_timing/min": timing_generate_min,
                "generation_timing/topk_ratio": timing_generate_topk_ratio,
            }
        ...
        # 回到 CPU，和 generate_sequences 对齐
        output = output.to("cpu")
